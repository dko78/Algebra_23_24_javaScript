...
const filtrirajZadatke = (e) => {
  ...
  zadaci.forEach((zadatak) => {
    const imeZadatka = zadatak.firstChild.textContent.toLowerCase(); //zadatak.firstChild.textContent.toLowerCase();
    //const imeZadatka1 = zadatak.textContent.toLowerCase();
...
//moje-lista svih zadataka
const sviZadaci = document.querySelectorAll(".pojedinacni-zadatak");

console.log(sviZadaci, " length ", sviZadaci.length);
//zato što je nodelist mozes foreach
sviZadaci.forEach((zadatak) => {
  console.log("1 ", zadatak.firstChild.textContent);
  console.log("2 ", zadatak.textContent);
});
